document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderAll() {
    ...
    studentsCache.forEach(student => {
      const studentName = (student.student ?? "Unknown")
  ...
  if (syncBtn) {
    syncBtn.addEventListener("click", () => {
      if (!confirm("⚠️ This may take some time. Sync after many changes for best performance.")) {
        return;
      }
  ...
  // 🔄 Sync Button
  document.getElementById("sync-students").addEventListener("click", () => {
    if (!confirm("⚠️ This may take some time. Sync after many changes for best performance.")) {
      return;
    }
    const delRequest = indexedDB.deleteDatabase(DB_NAME);
