export default function Teachers() {
    ...
    const teacherName = el('input', 'form-control'); teacherName.placeholder = 'Teacher Name';
    ...
    const dob = el('input', 'form-control'); dob.placeholder = 'DD/MM/YYYY'; dob.maxLength = 10; dob.inputMode = 'numeric';
    ...
    const role = el('input', 'form-control'); role.placeholder='Role (Teacher, Head, Principal)';
    ...
    saveBtn.addEventListener('click', async ()=>{
        ...
        if(!/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[0-2])\/\d{4}$/.test(dobVal)) return Toast.error('DOB must be DD/MM/YYYY');
        const [d,m,y] = dobVal.split('/').map(Number);
        const dateObj = new Date(y,m-1,d);
        if(dateObj.getDate()!==d || dateObj.getMonth()!==m-1 || dateObj.getFullYear()!==y) return Toast.error('Invalid Date of Birth');
        if(!/^\d{10}$/.test(contactVal)) return Toast.error('Contact must be 10 digits');
        ...
        try{
            ...
            if(res.status===200){
                ...
                [teacherName,fatherName,motherName,dob,contact,address,role].forEach(i=>i.value='');
